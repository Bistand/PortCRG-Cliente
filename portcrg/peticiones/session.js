...
export async function Resetpassword(email) {
  try {
    const res = await axios.post(
      "https://portcrg.onrender.com/api/user/forgotPass",
      { email: email }
    );
...
export async function newPassword(token, password) {
  try {
    const res = await axios.post(
      "https://portcrg.onrender.com/api/user/newPass",
      {
        token: token,
...
export async function datosUSer(tokenuser) {
  try {
    const usuario = await axios.get("https://portcrg.onrender.com/api/admin", {
      headers: {
